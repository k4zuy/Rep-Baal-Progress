...
class MinMaxVals:
    ...
    enhancer: MinMax = MinMax(0.1, 1.9)
...
ohl_color = TransformT("Color", _enhancer_impl(ImageEnhance.Color, 0.3, 0.9))
...
detail = TransformT("Detail", lambda pil_img, level: pil_img.filter(ImageFilter.DETAIL))
...
max_ = TransformT("Max", lambda pil_img, level: pil_img.filter(ImageFilter.MaxFilter))
min_ = TransformT("Min", lambda pil_img, level: pil_img.filter(ImageFilter.MinFilter))
median = TransformT(
...
def _mirrored_enhancer_impl(enhancer, minimum=None, maximum=None):
    ...
    def impl(pil_img, level):
        ...
        assert (
            mini == 0.0
        ), "This enhancer is used with a strength space that is mirrored around one."
        v = float_parameter(level, maxi - mini) + mini  # going to 0 just destroys it
        if random.random() < 0.5:
            v = -v
        return enhancer(pil_img).enhance(1.0 + v)

    return impl


mirrored_color = TransformT("Color", _mirrored_enhancer_impl(ImageEnhance.Color))
mirrored_contrast = TransformT(
    "Contrast", _mirrored_enhancer_impl(ImageEnhance.Contrast)
)
mirrored_brightness = TransformT(
    "Brightness", _mirrored_enhancer_impl(ImageEnhance.Brightness)
)
mirrored_sharpness = TransformT(
    "Sharpness", _mirrored_enhancer_impl(ImageEnhance.Sharpness)
)
...
cutout = TransformT(
    "Cutout",
    lambda img, l: CutoutDefault(
        img, int_parameter(l, img.size[0] * min_max_vals.cutout.max)
    ),
)
...
def blend(img1, v):
    if blend_images is None:
        print(
            "please set google_transformations.blend_images before using the enlarged_randaug search space."
        )
    i = np.random.choice(len(blend_images))
    img2 = blend_images[i]
    m = float_parameter(v, 0.4)
...
def set_augmentation_space(
    augmentation_space, num_strengths, custom_augmentation_space_augs=None
):
    ...
    if "wide" in augmentation_space:
        min_max_vals = MinMaxVals(
            shear=MinMax(0.0, 0.99),
            translate=MinMax(0, 32),
            rotate=MinMax(0, 135),
            solarize=MinMax(0, 256),
            posterize=MinMax(2, 8),
            enhancer=MinMax(0.01, 2.0),
            cutout=MinMax(0.0, 0.6),
        )
    elif ("uniaug" in augmentation_space) or ("randaug" in augmentation_space):
        min_max_vals = MinMaxVals(posterize=MinMax(4, 8), translate=MinMax(0, 14.4))
    elif "fixmirror" in augmentation_space:
        min_max_vals = MinMaxVals(posterize=MinMax(4, 8), enhancer=MinMax(0.0, 0.9))
    elif "fiximagenet" in augmentation_space:
        min_max_vals = MinMaxVals(posterize=MinMax(4, 8), translate=MinMax(0, 70))
    elif "fix" in augmentation_space:
        min_max_vals = MinMaxVals(posterize=MinMax(4, 8))
    elif "ohl" in augmentation_space:
        assert PARAMETER_MAX == 2
        min_max_vals = MinMaxVals(
            shear=MinMax(0.1, 0.3),
    ...
    if "xlong" in augmentation_space:
        ...
        ALL_TRANSFORMS = []
        ALL_TRANSFORMS += [
            custom_augmentation_space_augs_mapping[aug]
            for aug in custom_augmentation_space_augs
        ]
